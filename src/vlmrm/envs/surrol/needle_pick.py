...
from vlmrm.envs.surrol.psm_env import PsmEnv
from vlmrm.envs.surrol.robots.ecm import RENDER_HEIGHT, RENDER_WIDTH, FoV
from vlmrm.envs.surrol.utils.const import ASSET_DIR_PATH
from vlmrm.envs.surrol.robots.ecm import Ecm
...
class NeedlePick(PsmEnv):
    ...
    def __init__(self, render_mode=None, cid = -1):
        ...
        self.cid = cid
        print(f'cid in __init__: {self.cid}')
    ...
    def get_controller_action(self, obs) -> np.ndarray:
        """
        Define a human expert strategy
        """
        # four waypoints executed in sequential order
        ...
        # Get the events from the gamepad
        events = get_gamepad()

        for event in events:
            # Define your controller actions
            if event.code == 'ABS_Y':  # if left stick is moved up or down
                action[0] = event.state / 32768  # normalize to range [-1, 1]
            if event.code == 'ABS_X':  # if left stick is moved left or right
                action[1] = event.state / 32768  # normalize to range [-1, 1]
            if event.code == 'ABS_RZ':  # if right stick is moved up or down
                action[2] = event.state / 32768  # normalize to range [-1, 1]
            if event.code == 'ABS_Z':  # if right stick is moved left or right
                action[3] = event.state / 32768  # normalize to range [-1, 1]
